        ...
        for j in range(F):
            for k in range(H_out):
                for m in range(W_out):
                    ...
                    x_1 = m * stride
                    x_2 = x_1 + WW
                    window = x_pad[i, :, y_1:y_2, x_1:x_2]
        ...
        for j in range(F):
            for k in range(H_out):
                for m in range(W_out):
                    ...
                    x_1 = m * stride
                    x_2 = x_1 + WW
                    current_dout = dout[i, j, k, m]

                    db[j] += current_dout
                    
                    window = x_pad[i, :, y_1:y_2, x_1:x_2]
                    dw[j] += window * current_dout
                    
                    dx_pad[i, :, y_1:y_2, x_1:x_2] += w[j] * current_dout
        ...
        for j in range(C):
            for k in range(H_out):
                for m in range(W_out):
                    ...
                    dx[i, j, y_1:y_2, x_1:x_2] += mask * dout[i, j, k, m]
    return dx
